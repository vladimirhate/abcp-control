...
async function getCars(): Promise<Car[]> {
  ...
  // Берем данные за последние 2 года, чтобы собрать актуальную статистику
  const dateStart = new Date();
  dateStart.setFullYear(dateStart.getFullYear() - 2);
  
  return abcpRequest<Car[]>(
    "cp/garage",
    {
...
async function getUsersMap(): Promise<Record<string, string>> {
  const shop = await getShop();
  if (!shop) return {};
  const users = await abcpRequest<User[]>("cp/users", { limit: "1000", state: "1" }, {
...
export default async function GaragePage() {
  ...
  try {
    const [carsData, users] = await Promise.all([getCars(), getUsersMap()]);
    cars = carsData;
    usersMap = users;
  } catch (e) {
