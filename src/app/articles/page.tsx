...
export default async function ArticlesPage() {
  ...
  // Сортируем по количеству проданных штук (хиты продаж)
      ...
      <div className="mx-auto max-w-7xl">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Хиты продаж и маржинальность за последние 30 дней
          </p>
        </div>
            ...
            <div className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              {articles.length === 0 ? (
                <div className="p-6 text-center text-slate-500">
                  Нет данных по артикулам за этот период.
                </div>
              ) : (
                      ...
                      {articles.slice(0, 100).map((a) => (
                        <tr key={a.article + a.brand} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-slate-700">
                            {Math.round(a.revenue).toLocaleString("ru-RU")} ₽
                          </td>
                          <td className={`px-4 py-3 font-medium ${a.margin < 0 ? "text-red-600" : "text-green-700"}`}>
                            {Math.round(a.margin).toLocaleString("ru-RU")} ₽
                          </td>
                          <td className="px-4 py-3">
                            <span className={`font-medium ${
                              a.marginPercent >= 15 ? "text-green-700" :
                              a.marginPercent >= 5 ? "text-amber-600" :
                              "text-red-600"
                            }`}>
