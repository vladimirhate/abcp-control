...
export default async function ClientsPage({ searchParams }: PageProps) {
  ...
  const clientMap = new Map<string, { name: string; ordersCount: number; revenue: number; margin: number; debt: number }>();
  ...
  for (const order of orders) {
    ...
    if (existing) {
      ...
      clientMap.set(id, { name, ordersCount: 1, revenue: sum, margin: orderMargin, debt: debt });
    }
  }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Топ клиентов и должников за выбранный период</p>
          </div>
          ...
          <>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-xl border border-red-200 bg-red-50 p-5 shadow-sm">
                ...
                <div className="mt-2 text-3xl font-bold text-red-700">{Math.round(totalDebt).toLocaleString("ru-RU")} ₽</div>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm">
              ...
              {clients.length === 0 ? (
                <div className="p-6 text-center text-slate-500">Нет данных по клиентам за этот период.</div>
              ) : (
                    ...
                    <tbody>
                      {clients.slice(0, 100).map((c, idx) => (
                        <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-slate-700">{Math.round(c.revenue).toLocaleString("ru-RU")} ₽</td>
                          <td className={`px-4 py-3 font-medium ${c.margin < 0 ? "text-red-600" : "text-green-700"}`}>{Math.round(c.margin).toLocaleString("ru-RU")} ₽</td>
                          <td className="px-4 py-3">
                            <span className={`font-medium ${c.marginPercent >= 15 ? "text-green-700" : c.marginPercent >= 5 ? "text-amber-600" : "text-red-600"}`}>
